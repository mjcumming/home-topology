...
class TestConfiguration:
    ...
    def test_default_config(self, ambient_module):
        """Test default configuration."""
        config = ambient_module.default_config()
        
        assert config["version"] == 1
        assert config["auto_discover"] is True
        assert config["inherit_from_parent"] is True
        assert config["dark_threshold"] == 50.0
        assert config["bright_threshold"] == 500.0
        assert config["fallback_to_sun"] is True
        assert config["assume_dark_on_error"] is True
    
    def test_config_schema(self, ambient_module):
        """Test configuration schema."""
        schema = ambient_module.location_config_schema()
        
        assert schema["type"] == "object"
        assert "lux_sensor" in schema["properties"]
        assert "dark_threshold" in schema["properties"]
        assert "bright_threshold" in schema["properties"]
    
    def test_set_lux_sensor(self, attached_ambient_module, loc_manager):
        """Test setting lux sensor for location."""
...
class TestSensorDetection:
    ...
    def test_explicit_sensor_overrides_discovery(
        self, attached_ambient_module, loc_manager
    ):
...
class TestHierarchicalLookup:
    ...
    def test_local_sensor_preferred(
        self, attached_ambient_module, loc_manager, platform_adapter
    ):
    ...
    def test_inherit_from_grandparent(
        self, attached_ambient_module, loc_manager, platform_adapter
    ):
    ...
    def test_no_inherit_when_disabled(
        self, attached_ambient_module, loc_manager, platform_adapter
    ):
        ...
        loc_manager.set_module_config("kitchen", "ambient", {
...
class TestFallbackStrategies:
    ...
    def test_sun_fallback_below_horizon(
        self, attached_ambient_module, platform_adapter
    ):
    ...
    def test_sun_fallback_above_horizon(
        self, attached_ambient_module, platform_adapter
    ):
